...
import {backend} from './backend-v5.js';
import {$,$$,esc,fmtDate,timeAgo,uid,nowIso,normalizeUrl,sourceFromUrl,isDirectVideo,isHls,parseTags,shortcutMatches,prettyShortcut,downloadBlob,pickFile,sortBy,randomSubset} from './utils-v5.js';
...
function applySettings(){activeLimitTop.value=String(S.settings.maxActiveTabs||3);localStorage.setItem('nox_panic_shortcut',S.settings.panicShortcut||'Ctrl+Shift+Space');$('#panic-shortcut-label').textContent=prettyShortcut(S.settings.panicShortcut||'Ctrl+Shift+Space');document.body.classList.toggle('blur-media',!!S.settings.blurThumbnails)}
...
async function loadAppData(){S.settings=await backend.getSettings();S.vaults=await backend.listVaults();S.currentVaultId=localStorage.getItem('nox_current_vault')||S.vaults[0]?.id||null;if(S.currentVaultId&&!S.vaults.some(v=>v.id===S.currentVaultId))S.currentVaultId=S.vaults[0]?.id||null;await loadVaultData();refreshVaultSwitcher();applySettings()}
...
function thumb(){return `<div class="thumb"></div>`}
...
async function selectVault(id){S.currentVaultId=id;localStorage.setItem('nox_current_vault',id);await loadVaultData();refreshVaultSwitcher();if(!S.unlocked.has(id)){showVaultLock(id);return}await backend.touchVault(id);render()}
...
function normalizeSession(s){const tabs=(s.tabs||[]).map(t=>({id:t.id||uid(),url:t.url||'',title:t.title||sourceFromUrl(t.url),embedUrl:t.embedUrl||'',mediaUrl:t.mediaUrl||'',thumbnail:t.thumbnail||'',status:t.status||'suspended',pinned:!!t.pinned,lastActiveAt:t.lastActiveAt||0,tags:t.tags||[],categoryIds:t.categoryIds||[]}));let active=tabs.filter(t=>t.status==='active');if(!active.length&&tabs.length){tabs.slice(0,Math.min(S.settings.maxActiveTabs,tabs.length)).forEach(t=>t.status='active')}active=tabs.filter(t=>t.status==='active');if(active.length>S.settings.maxActiveTabs){active.slice(S.settings.maxActiveTabs).forEach(t=>t.status='suspended')}return {...s,tabs}}
async function openSessionPage(id){let s=byId(S.sessions,id);if(!s){toast('Session not found.','error');routeTo('sessions');return}if(!S.activeSession||S.activeSession.id!==id){unloadFrames();S.activeSession=normalizeSession({...s,tabs:(s.tabs||[]).map(t=>({...t}))});S.currentTabId=S.activeSession.tabs.find(t=>t.status==='active')?.id||S.activeSession.tabs[0]?.id||null;S.activeSession.lastOpenedAt=nowIso();await saveActiveSession(false)}renderSessionView();}
...
function renderSessionView(){const s=S.activeSession;if(!s){routeTo('sessions');return}const current=byId(s.tabs,S.currentTabId);page.innerHTML=`${pageHeader(`Session: ${esc(s.name)}`,`${s.tabs.length} tabs · ${s.tabs.filter(t=>t.status==='active').length} active · ${s.tabs.filter(t=>t.status==='suspended').length} suspended`,`<button class="btn ghost" data-save-all>Save All</button><button class="btn ghost" data-suspend-all>Suspend All</button><button class="btn primary" data-add-tab>＋ Tab</button>`) }
  <div class="session-layout">
    <section class="card browser-card">
      ...
      <div class="address-row"><button class="btn icon small" data-back>←</button><button class="btn icon small" data-reload>↻</button><input id="session-url" type="url" placeholder="Enter a URL, direct video, or embeddable page…" value="${esc(current?.url||'')}"><button class="btn small primary" data-go>Go</button></div>
      <div class="browser-stage" id="browser-stage"><div class="browser-placeholder"><div><img src="./assets/nox-mark.svg" width="70" alt=""><h2>${current?'Loading current tab…':'Open a tab'}</h2><p>Pages that allow embedding can be viewed here. Direct MP4/WebM video links use the built-in player. Some third-party sites intentionally block iframe embedding; Nox Vault cannot override that browser security policy.</p></div></div></div>
      <div class="browser-toolbar"><button class="btn small primary" data-save-tab>☆ Save to Vault</button><button class="btn small ghost" data-metadata>Suggest Title & Tags</button><button class="btn small ghost" data-note-tab>✎ Note</button><button class="btn small ghost" data-pin-tab>${current?.pinned?'Unpin':'Pin'}</button><div class="tab-dots"><span class="dot active"></span> Active ${s.tabs.filter(t=>t.status==='active').length}<span class="dot suspended"></span> Suspended ${s.tabs.filter(t=>t.status==='suspended').length}</div></div>
    </section>
    <aside class="session-side">
      <section class="card side-section"><h3>Current Tab</h3>${current?`<div class="tab-info"><strong>${esc(current.title||current.url)}</strong><small class="muted mono">${esc(current.url)}</small><div class="tag-cloud">${(current.tags||[]).slice(0,8).map(t=>badge(t,'blue')).join('')||'<span class="muted smalltext">No tags detected yet</span>'}</div><div class="spaced"><span>${badge(current.status,current.status==='active'?'green':'purple')}</span><button class="btn small ghost" data-toggle-active>${current.status==='active'?'Suspend':'Activate'}</button></div></div>`:'<p class="muted">No current tab.</p>'}</section>
...
async function suspendTab(id){const t=byId(S.activeSession?.tabs,id);if(!t)return;t.status='suspended';const f=S.tabFrames.get(id);if(f){try{f.src='about:blank';f.remove()}catch{}S.tabFrames.delete(id)}if(S.currentTabId===id)S.currentTabId=S.activeSession.tabs.find(x=>x.status==='active')?.id||id;await saveActiveSession(false);renderSessionView()}
...
async function closeTab(id){const s=S.activeSession;if(!s)return;const idx=s.tabs.findIndex(x=>x.id===id);if(idx<0)return;const f=S.tabFrames.get(id);if(f){f.src='about:blank';f.remove();S.tabFrames.delete(id)}s.tabs.splice(idx,1);if(S.currentTabId===id)S.currentTabId=s.tabs[Math.max(0,idx-1)]?.id||s.tabs[0]?.id||null;await saveActiveSession(false);renderSessionView()}
function addTabModal(){modal(`<h2>Add Tab</h2><p>Enter a page URL, official embed URL, direct MP4/WebM URL, or HLS source.</p><form class="form-grid"><label class="full">URL<input name="url" type="url" required placeholder="https://example.com/video"></label><label>Optional title<input name="title" placeholder="Auto-detect later"></label><label>Optional embed URL<input name="embedUrl" type="url" placeholder="https://…/embed/…"></label><div class="form-actions full"><button class="btn primary">Add Tab</button></div></form>`,{onOpen:r=>r.querySelector('form').onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));const t={id:uid(),url:d.url,title:d.title||sourceFromUrl(d.url),embedUrl:d.embedUrl||'',mediaUrl:isDirectVideo(d.url)||isHls(d.url)?d.url:'',status:'suspended',pinned:false,lastActiveAt:0,tags:[],categoryIds:[]};S.activeSession.tabs.push(t);r.remove();await activateTab(t.id)}})}
async function navigateCurrent(url){const t=byId(S.activeSession?.tabs,S.currentTabId);if(!t||!url)return;t.url=url;t.mediaUrl=isDirectVideo(url)||isHls(url)?url:'';t.embedUrl='';t.title=sourceFromUrl(url);t.status='active';t.lastActiveAt=Date.now();const f=S.tabFrames.get(t.id);if(f){try{f.src='about:blank';f.remove()}catch{}S.tabFrames.delete(t.id)}await saveActiveSession(false);renderSessionView()}
...
async function metadataIntoTab(tab){toast('Detecting page metadata…');try{const m=await backend.fetchMetadata(tab.url);Object.assign(tab,{title:m.title||tab.title,thumbnail:m.thumbnail||tab.thumbnail,embedUrl:m.embedUrl||tab.embedUrl,mediaUrl:m.mediaUrl||tab.mediaUrl,tags:[...new Set([...(tab.tags||[]),...(m.tags||[])])].slice(0,30)});await saveActiveSession(false);renderSessionView();toast('Metadata suggestions added.','success')}catch(err){toast(`Metadata: ${err.message}`,'error')}}
...
function mediaCard(i){return `<article class="media-card" data-item-card="${i.id}" data-cats="${esc((i.categories||[]).join(','))}" data-fav="${i.favorite?'1':'0'}" data-viewed="${i.lastViewedAt?'1':'0'}"><button class="star ${i.favorite?'on':''}" data-star="${i.id}">${i.favorite?'★':'☆'}</button><div class="thumb"></div><div class="media-body"><h3>${esc(i.title)}</h3><p>${esc(i.source||'link')} · ${(i.tags||[]).slice(0,3).map(esc).join(' · ')}</p><div class="media-meta"><span>${i.viewCount||0} views · ${timeAgo(i.lastViewedAt||i.createdAt)}</span><button class="btn small ghost" data-item-menu="${i.id}">•••</button></div></div></article>`}
...
async function openItemInSession(id){const i=byId(S.items,id);if(!i)return;const s=await backend.saveSession(S.currentVaultId,{name:i.title,tabs:[{id:uid(),url:i.url,title:i.title,embedUrl:i.embedUrl||'',mediaUrl:i.mediaUrl||'',thumbnail:i.thumbnail||'',tags:i.tags||[],status:'active',pinned:false,lastActiveAt:Date.now()}],categoryIds:i.categories||[]});S.sessions=await backend.listSessions(S.currentVaultId);routeTo(`session/${s.id}`)}
  ...
  <div class="player-layout">
    ...
    <aside class="card playlist">
      ...
      ${S.playerQueue.length
        ? S.playerQueue.map((entry,index)=>`<button class="playlist-row ${index===S.playerIndex?'current':''}" data-play-index="${index}"><span>${index+1}</span><div class="mini-thumb"></div><div><b>${esc(entry.title)}</b><small>${esc(entry.source||sourceFromUrl(entry.url))}</small></div></button>`).join('')
...
function playerStage(i){if(!i)return `<div class="browser-placeholder"><div><img src="./assets/nox-mark.svg" width="70"><h2>No item selected</h2><p>Choose a source on the left.</p></div></div>`;if(isDirectVideo(i.mediaUrl||i.url)||isHls(i.mediaUrl||i.url))return `<video id="playlist-video" controls preload="auto" src="${esc(i.mediaUrl||i.url)}"></video>`;return `<iframe id="playlist-frame" src="${esc(i.embedUrl||i.url)}" allow="autoplay; fullscreen; picture-in-picture" referrerpolicy="no-referrer"></iframe>`}
...
function renderSettings(){page.innerHTML=`${pageHeader('Settings','Control sessions, privacy, player behaviour, backups and appearance.')}<div class="settings-tabs"><button class="active">General</button><button>Sessions</button><button>Privacy</button><button>Player</button></div><div class="settings-grid">
  <section class="card setting-group"><h3>Sessions & Memory</h3><div class="setting-row"><div><strong>Maximum active tabs</strong><p>Only this many tabs keep an iframe/player mounted.</p></div><select id="set-active-limit" class="select">${[1,2,3,4,5].map(n=>`<option value="${n}" ${n===Number(S.settings.maxActiveTabs)?'selected':''}>${n}</option>`).join('')}</select></div><div class="setting-row"><div><strong>Session autosave</strong><p>Save tab changes as you browse.</p></div><input id="set-autosave" class="toggle" type="checkbox" ${S.settings.sessionAutosave?'checked':''}></div><div class="setting-row"><div><strong>Preload active videos</strong><p>Keep active direct-video tabs preloaded.</p></div><input id="set-preload" class="toggle" type="checkbox" ${S.settings.preloadVideos?'checked':''}></div><div class="setting-row"><div><strong>Restore last session</strong><p>Reserved for a future launch preference.</p></div><input id="set-restore" class="toggle" type="checkbox" ${S.settings.restoreLastSession?'checked':''}></div></section>
  <section class="card setting-group"><h3>Privacy</h3><div class="setting-row"><div><strong>Auto-lock after inactivity</strong><p>Locks the active vault and unloads active frames.</p></div><select id="set-inactivity" class="select">${[1,5,10,15,30,60].map(n=>`<option value="${n}" ${n===Number(S.settings.inactivityMinutes)?'selected':''}>${n} min</option>`).join('')}</select></div><div class="setting-row"><div><strong>Show decoy when locking</strong><p>Switch to the Knowledge Hub page on lock.</p></div><input id="set-decoy-lock" class="toggle" type="checkbox" ${S.settings.decoyOnLock?'checked':''}></div><div class="setting-row"><div><strong>Blur thumbnails</strong><p>Blur library and image previews until you disable it.</p></div><input id="set-blur" class="toggle" type="checkbox" ${S.settings.blurThumbnails?'checked':''}></div><div class="setting-row"><div><strong>Panic shortcut</strong><p>Works while Nox Vault or the decoy page has focus.</p></div><button id="capture-shortcut" class="btn ghost">${prettyShortcut(S.settings.panicShortcut)}</button></div></section>
  <section class="card setting-group"><h3>Player</h3><div class="setting-row"><div><strong>Auto-next direct videos</strong><p>Advance after an HTML5 video ends.</p></div><input id="set-player-auto" class="toggle" type="checkbox" ${S.settings.playerAutoplay?'checked':''}></div><div class="setting-row"><div><strong>Default shuffle</strong><p>Start player queues shuffled.</p></div><input id="set-shuffle" class="toggle" type="checkbox" ${S.settings.playerShuffle?'checked':''}></div></section>
  ...
  <section class="card setting-group danger-zone"><h3>Vault Security</h3><p class="muted smalltext">Vault passwords are hashed locally with PBKDF2 and the salted hash is stored in your owner-only Firestore vault record. No Cloud Function is required. This is an application lock, not encryption.</p><div class="quick-grid"><button class="quick" data-change-pass><strong>🔑 Change Vault Password</strong><small>Current vault</small></button><button class="quick" data-lock-now><strong>🔒 Lock Now</strong><small>Unload active content</small></button></div></section>
  </div>`;
  const save=async(p)=>{S.settings={...S.settings,...p};await backend.saveSettings(p);applySettings();resetIdle()};$('#set-active-limit').onchange=e=>updateActiveLimit(Number(e.target.value));$('#set-autosave').onchange=e=>save({sessionAutosave:e.target.checked});$('#set-preload').onchange=e=>save({preloadVideos:e.target.checked});$('#set-restore').onchange=e=>save({restoreLastSession:e.target.checked});$('#set-inactivity').onchange=e=>save({inactivityMinutes:Number(e.target.value)});$('#set-decoy-lock').onchange=e=>save({decoyOnLock:e.target.checked});$('#set-blur').onchange=e=>save({blurThumbnails:e.target.checked});$('#set-player-auto').onchange=e=>save({playerAutoplay:e.target.checked});$('#set-shuffle').onchange=e=>save({playerShuffle:e.target.checked});$('[data-export]',page).onclick=exportCurrentVault;$('[data-import]',page).onclick=importVaultJson;$('[data-change-pass]',page).onclick=()=>changeVaultPasswordModal(S.currentVaultId);$('[data-lock-now]',page).onclick=()=>lockVault(S.currentVaultId,{decoy:false});$('#capture-shortcut').onclick=captureShortcut;}
...
function renderPrivacy(){page.innerHTML=`${pageHeader('Privacy & Decoy','Instantly hide Nox Vault, lock vaults and control idle behaviour.')}<div class="settings-grid"><section class="card setting-group"><h3>Panic Hide</h3><div class="notice">Current shortcut: <b>${prettyShortcut(S.settings.panicShortcut)}</b>. The shortcut works while this page has keyboard focus.</div><div style="height:12px"></div><button class="btn danger wide" data-panic-now>◉̸ Test Panic Hide</button><p class="muted smalltext">Panic Hide unloads Nox Vault frames by navigating to the decoy page. Use the same shortcut on the decoy page to return to Nox Vault.</p></section><section class="card setting-group"><h3>Automatic Lock</h3><div class="status-list"><div class="status-line"><span>Idle timeout</span><b>${S.settings.inactivityMinutes} minutes</b></div><div class="status-line"><span>Decoy on lock</span><b>${S.settings.decoyOnLock?'Yes':'No'}</b></div><div class="status-line"><span>Active frames unloaded</span><b>Yes</b></div><div class="status-line"><span>Vault passwords</span><b>PBKDF2 hashed</b></div></div><div style="height:12px"></div><button class="btn outline wide" data-lock-all>Lock All Vaults</button></section><section class="card setting-group"><h3>Decoy Preview</h3><div class="thumb" style="height:180px;filter:none;background:linear-gradient(160deg,#4a6c8c,#e5a36b 60%,#24394f)"></div><p class="muted smalltext">The default decoy is a neutral Knowledge Hub dashboard. You can customize decoy.html directly if you want a different appearance.</p><button class="btn ghost" data-open-decoy>Open Decoy Page</button></section><section class="card setting-group"><h3>Important</h3><p class="muted smalltext">Nox Vault does not use client-side encryption in this build, by design. Firebase Authentication and owner-only Security Rules protect access to stored records; vault passwords add an application-level lock.</p></section></div>`;$('[data-panic-now]',page).onclick=panic;$('[data-lock-all]',page).onclick=()=>lockAll({decoy:false});$('[data-open-decoy]',page).onclick=()=>window.open('./decoy.html','_self')}

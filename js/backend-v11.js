...
import { OWNER_UID } from './firebase-config.js?v=20260915-v11-vault-options';
import { DEFAULT_CATEGORIES,uid,nowIso,normalizeUrl,sha256,sourceFromUrl } from './utils-v8.js';
...
function demoSeed(){
  ...
  return {user:{uid:'demo-owner',email:'demo@nox.local'},settings:defaultSettings,vaults:{
    [v1]:{id:v1,name:'Personal',description:'Main private collection',color:'#3377ff',nsfw:false,hasPassword:true,createdAt:nowIso(),lastOpenedAt:nowIso()},
    [v2]:{id:v2,name:'Archive',description:'Saved references',color:'#6758ff',nsfw:false,hasPassword:true,createdAt:nowIso(),lastOpenedAt:null}
...
async function makePasswordRecord(password){
  ...
  if(!value)return {hasPassword:false,lockSalt:null,lockHash:null,lockIterations:null,lockVersion:null};
  if(value.length<2)throw new Error('Vault password must be at least 2 characters, or leave it blank for no password.');
  const salt=randomBytes(16);
  return {hasPassword:true,lockSalt:bytesToBase64(salt),lockHash:await derivePasswordHash(value,salt),lockIterations:PASSWORD_ITERATIONS,lockVersion:1};
}
async function verifyPasswordRecord(vault,password){
  if(!vault?.lockHash||!vault?.lockSalt)return true;
  const value=String(password??'');
  if(value.length<2)return false;
  const hash=await derivePasswordHash(value,base64ToBytes(vault.lockSalt),Number(vault.lockIterations||PASSWORD_ITERATIONS));
...
export const backend={
  ...
  async createVault({name,password='',description='',color='#3377ff',nsfw=false}){
    const value=String(password??'');
    if(value&&value.length<2)throw new Error('Vault password must be at least 2 characters, or leave it blank for no password.');
    if(demoMode){const id=uid();demo.vaults[id]={id,name,description,color,nsfw:!!nsfw,hasPassword:!!value,createdAt:nowIso(),lastOpenedAt:null};demo.vaultSecrets[id]=value;['categories','items','sessions','notes','images','trash'].forEach(k=>demo[k][id]={});DEFAULT_CATEGORIES.forEach(([n,c],i)=>demo.categories[id][`cat-${i}`]={id:`cat-${i}`,name:n,color:c,createdAt:nowIso()});saveDemo(demo);return id}
    const id=uid(),lock=await makePasswordRecord(value);
    await setDoc(doc(db,...userRoot(),'vaults',id),{id,name,description,color,nsfw:!!nsfw,createdAt:serverTimestamp(),lastOpenedAt:null,...lock});
    for(const [n,c] of DEFAULT_CATEGORIES)await this.saveCategory(id,{name:n,color:c});
    return id;
  },
  async verifyVault(vaultId,password=''){
    if(demoMode){const vault=demo.vaults[vaultId];if(!vault)return false;if(!vault.hasPassword)return true;return demo.vaultSecrets[vaultId]===String(password??'')}
  ...
  async changeVaultPassword(vaultId,password=''){
    const value=String(password??'');
    if(value&&value.length<2)throw new Error('Vault password must be at least 2 characters, or leave it blank to remove the password.');
    if(demoMode){demo.vaultSecrets[vaultId]=value;if(demo.vaults[vaultId])demo.vaults[vaultId].hasPassword=!!value;saveDemo(demo);return}
    await updateDoc(doc(db,...userRoot(),'vaults',vaultId),await makePasswordRecord(value));
  },

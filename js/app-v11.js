// Nox Vault v1.3.3 optional vault passwords + per-vault NSFW + legacy companion
import {backend} from './backend-v11.js';
...
function applyVaultPrivacy(){
  const v=currentVault();
  document.body.classList.toggle('vault-nsfw',Boolean(v?.nsfw));
  const lock=$('#lock-btn');
  if(lock){lock.disabled=Boolean(v&&!vaultHasPassword(v));lock.title=v&&!vaultHasPassword(v)?'This vault has no password. Use Panic Hide for privacy.':'';}
}
...
function refreshVaultSwitcher(){vaultSwitcher.innerHTML=S.vaults.map(v=>`<option value="${esc(v.id)}">${esc(v.name)}${vaultHasPassword(v)&&!S.unlocked.has(v.id)?' 🔒':''}</option>`).join('')||'<option>No vaults</option>';if(S.currentVaultId)vaultSwitcher.value=S.currentVaultId;applyVaultPrivacy()}
...
async function loadAppData(){
  ...
  for(const v of S.vaults)if(!vaultHasPassword(v))S.unlocked.add(v.id);
...
function lockVault(vaultId,{decoy=false}={}){
  if(!vaultId)return;
  const v=byId(S.vaults,vaultId);
  unloadFrames();
  if(vaultId===S.currentVaultId){S.activeSession=null;S.currentTabId=null}
  if(v&&!vaultHasPassword(v)){
    S.unlocked.add(vaultId);
    refreshVaultSwitcher();
    if(decoy||S.settings?.decoyOnLock)location.href='./decoy.html';
    else toast('This vault has no password, so there is nothing to lock. Use Panic Hide to conceal it.');
    return;
  }
  S.unlocked.delete(vaultId);
  refreshVaultSwitcher();
  if(decoy||S.settings?.decoyOnLock)location.href='./decoy.html';else showVaultLock(vaultId);
}
function lockAll(opts={}){
  for(const v of S.vaults){if(vaultHasPassword(v))S.unlocked.delete(v.id);else S.unlocked.add(v.id)}
  unloadFrames();refreshVaultSwitcher();
  if(opts.decoy||S.settings?.decoyOnLock)location.href='./decoy.html';else render();
}
function showVaultLock(vaultId){
  const v=byId(S.vaults,vaultId);if(!v)return;
  if(!vaultHasPassword(v)){S.unlocked.add(vaultId);backend.touchVault(vaultId).catch(()=>{});refreshVaultSwitcher();render();return}
  const old=$('.lock-overlay');old?.remove();
  const wrap=document.createElement('div');wrap.className='lock-overlay';
  wrap.innerHTML=`<div class="lock-card glass"><img src="./assets/nox-mark.svg" alt=""><h2>${esc(v.name)}</h2><p>This vault is locked. Enter its vault password to continue.</p><form><input type="password" name="password" autocomplete="current-password" placeholder="Vault password" minlength="2" required autofocus><button class="btn primary" type="submit">Unlock Vault</button><button class="btn ghost" type="button" data-vaults>Back to Vaults</button><p class="form-message"></p></form></div>`;
  document.body.append(wrap);
  const input=wrap.querySelector('input[name=password]');requestAnimationFrame(()=>input?.focus());
  wrap.querySelector('[data-vaults]').onclick=()=>{wrap.remove();routeTo('vaults')};
  wrap.querySelector('form').onsubmit=async e=>{e.preventDefault();const btn=e.submitter,msg=wrap.querySelector('.form-message');setBusy(btn,true,'Checking…');try{const ok=await backend.verifyVault(vaultId,e.target.password.value);if(!ok)throw new Error('Incorrect vault password.');S.unlocked.add(vaultId);await backend.touchVault(vaultId);wrap.remove();refreshVaultSwitcher();render()}catch(err){msg.textContent=err.message}finally{setBusy(btn,false)}};
}
async function ensureUnlocked(){if(!S.currentVaultId)return false;const v=currentVault();if(v&&!vaultHasPassword(v)){S.unlocked.add(v.id);return true}if(S.unlocked.has(S.currentVaultId))return true;showVaultLock(S.currentVaultId);return false}
...
function renderVaults(){
  ...
  <div class="vault-grid">${S.vaults.map(v=>{const protectedVault=vaultHasPassword(v),open=S.unlocked.has(v.id)||!protectedVault;return `<section class="card vault-card ${v.id===S.currentVaultId?'selected':''}"><div class="spaced"><div class="vault-lock">${protectedVault?(open?'🔓':'🔒'):'○'}</div>${badge(protectedVault?(open?'Active':'Locked'):'No password',protectedVault?(open?'green':'red'):'blue')}</div><h3>${esc(v.name)}</h3><p>${esc(v.description||'Private vault')}</p><div class="tag-cloud">${v.nsfw?badge('NSFW blur','purple'):''}</div><div class="vault-actions"><small class="muted">${timeAgo(v.lastOpenedAt)}</small><div><button class="btn small ${open?'outline':'primary'}" data-vault-open="${v.id}">${open?'Open':'Unlock'} →</button> <button class="btn small ghost" data-vault-menu="${v.id}">•••</button></div></div></section>`}).join('')}<button class="card vault-card create-card" data-new-vault><div><b>＋</b><h3>New Vault</h3><small>Create another private space</small></div></button></div>`;
...
async function selectVault(id){S.currentVaultId=id;localStorage.setItem('nox_current_vault',id);const v=byId(S.vaults,id);if(v&&!vaultHasPassword(v))S.unlocked.add(id);await loadVaultData();refreshVaultSwitcher();history.replaceState(null,'','#dashboard');if(!S.unlocked.has(id)){showVaultLock(id);return}await backend.touchVault(id);applyVaultPrivacy();render()}
function newVaultModal(){modal(`<h2>Create New Vault</h2><p>A password is optional. If you use one, it can be as short as 2 characters. Vault passwords are application locks, not encryption.</p><form id="vault-form" class="form-grid"><label>Vault name<input name="name" required maxlength="60" placeholder="Personal"></label><label>Accent colour<input name="color" type="color" value="#3377ff"></label><label class="full">Description<input name="description" maxlength="140" placeholder="Optional description"></label><label class="full">Vault password <span class="muted">(optional)</span><input name="password" type="password" minlength="2" autocomplete="new-password" placeholder="Leave blank for no password"></label><label class="full checkline"><input name="nsfw" type="checkbox"> <span><strong>NSFW</strong><small class="muted">Blur thumbnails and stored images whenever this vault is open.</small></span></label><div class="form-actions full"><button class="btn ghost" type="button" data-close>Cancel</button><button class="btn primary" type="submit">Create Vault</button></div></form>`,{onOpen:r=>{r.querySelector('[data-close]').onclick=()=>r.remove();r.querySelector('form').onsubmit=async e=>{e.preventDefault();const b=e.submitter;setBusy(b,true,'Creating…');try{const d=Object.fromEntries(new FormData(e.target));d.nsfw=e.target.nsfw.checked;const id=await backend.createVault(d);S.vaults=await backend.listVaults();S.currentVaultId=id;S.unlocked.add(id);localStorage.setItem('nox_current_vault',id);await loadVaultData();r.remove();refreshVaultSwitcher();toast('Vault created.','success');render()}catch(err){toast(err.message,'error')}finally{setBusy(b,false)}}}})}
function vaultMenu(e,id){openContext(e,[['Rename / edit',()=>editVaultModal(id)],['Change / remove password',()=>changeVaultPasswordModal(id)],['Lock',()=>lockVault(id,{decoy:false})],['Delete vault',async()=>{if(await confirmModal('Delete vault?','This permanently removes the vault and all Firestore records inside it, including stored image chunks.','Delete Vault')){await backend.deleteVault(id);S.unlocked.delete(id);S.vaults=await backend.listVaults();if(S.currentVaultId===id){S.currentVaultId=S.vaults[0]?.id||null;localStorage.setItem('nox_current_vault',S.currentVaultId||'');if(currentVault()&&!vaultHasPassword(currentVault()))S.unlocked.add(S.currentVaultId);await loadVaultData()}refreshVaultSwitcher();render();toast('Vault deleted.')}}]])}
function editVaultModal(id){const v=byId(S.vaults,id);modal(`<h2>Edit Vault</h2><form class="form-grid"><label>Vault name<input name="name" value="${esc(v.name)}" required></label><label>Accent colour<input name="color" type="color" value="${esc(v.color||'#3377ff')}"></label><label class="full">Description<input name="description" value="${esc(v.description||'')}"></label><label class="full checkline"><input name="nsfw" type="checkbox" ${v.nsfw?'checked':''}> <span><strong>NSFW</strong><small class="muted">Blur this vault's thumbnails and stored images.</small></span></label><div class="form-actions full"><button class="btn primary">Save</button></div></form>`,{onOpen:r=>r.querySelector('form').onsubmit=async e=>{e.preventDefault();const patch=Object.fromEntries(new FormData(e.target));patch.nsfw=e.target.nsfw.checked;await backend.updateVault(id,patch);S.vaults=await backend.listVaults();r.remove();refreshVaultSwitcher();applyVaultPrivacy();render();toast('Vault updated.','success')}})}
function changeVaultPasswordModal(id){const v=byId(S.vaults,id);modal(`<h2>${vaultHasPassword(v)?'Change / Remove':'Add'} Vault Password</h2><p>Passwords are optional. Enter at least 2 characters, or leave the field blank to remove the password entirely.</p><form><input name="password" type="password" minlength="2" placeholder="${vaultHasPassword(v)?'New password, or blank to remove':'Optional password'}" autocomplete="new-password"><div class="form-actions"><button class="btn primary">Save Password Setting</button></div></form>`,{onOpen:r=>r.querySelector('form').onsubmit=async e=>{e.preventDefault();const b=e.submitter;setBusy(b,true);try{const value=e.target.password.value;await backend.changeVaultPassword(id,value);S.vaults=await backend.listVaults();const updated=byId(S.vaults,id);if(!vaultHasPassword(updated))S.unlocked.add(id);r.remove();refreshVaultSwitcher();toast(value?'Vault password updated.':'Vault password removed.','success');render()}catch(err){toast(err.message,'error')}finally{setBusy(b,false)}}})}
...
function extensionRequest(type,payload={},timeout=3500){
  return new Promise((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{cleanup();reject(new Error('Nox Vault Companion extension did not respond. Open the Chrome extension in-page sidebar and make sure it is installed.'))},timeout);
    const handler=e=>{
...
function renderSessionView(){
  ...
  <div class="extension-session-layout">
    <section class="card extension-browser-card">
      ...
      <div class="notice extension-notice"><b>Nox Companion mode:</b> the actual website opens normally in Chrome, so its player, related videos, navigation and cookies work as intended. The extension in-page sidebar stays beside it and can save metadata, tags, related links and sessions without iframe/CORS restrictions.</div>
    </section>
...
async function metadataIntoTab(tab,announce=true){if(announce)toast('Use the Nox Chrome Companion in-page sidebar to scan the live page metadata.','success');return tab}
...
function renderExtension(){
  page.innerHTML=`${pageHeader('Browser Companion','Use the Nox Chrome extension for real website playback, metadata detection, related videos and RAM-conscious sessions. This legacy-compatible build injects Nox directly into the current webpage instead of using Chrome 114+ Side Panel APIs.')}
  <div class="settings-grid"><section class="card setting-group"><h3>Why the extension</h3><p class="muted">Normal third-party pages are no longer loaded inside Nox iframes. The Legacy Chrome Companion injects a Nox sidebar into the real website, so videos and site navigation behave normally even on older Chrome.</p><button class="btn primary" data-ext-check>Check Companion</button><div class="notice" data-ext-status style="margin-top:12px">Not checked yet.</div></section><section class="card setting-group"><h3>Session memory model</h3><p class="muted">A saved session can contain dozens of URLs while Chrome keeps only 1–5 live page slots. Moving to another item reuses an old slot, keeping RAM usage predictable.</p><div class="status-list"><div class="status-line"><span>Your limit</span><b>${S.settings.maxActiveTabs}</b></div><div class="status-line"><span>Viewing</span><b>Real website</b></div><div class="status-line"><span>Metadata</span><b>Content script</b></div></div></section><section class="card setting-group"><h3>Install / update Chrome Companion</h3><p class="muted">Download the current extension ZIP directly from Nox. Extract it, then open <b>chrome://extensions</b>, enable Developer mode, choose <b>Load unpacked</b>, and select the extracted <code>nox-vault-companion-extension-legacy</code> folder.</p><div class="form-actions" style="justify-content:flex-start"><a class="btn primary" href="./downloads/nox-vault-companion-extension-legacy-v1.3.3.zip" download>⇩ Download Legacy Chrome Companion (.zip)</a><button class="btn ghost" data-ext-options>Open Extension Settings</button></div><p class="muted smalltext" style="margin-top:10px">If Chrome already has Nox loaded, extract the new ZIP over the old folder and click <b>Reload</b> on the extension card in <b>chrome://extensions</b>.</p></section></div>`;
...
function renderSettings(){
  ...
  <div class="settings-grid">
    ...
    <section class="card setting-group"><h3>Chrome Companion</h3>
      <p class="muted smalltext">Playback and live metadata scanning happen beside the real website through the Nox Vault Companion extension. Download it here whenever you install Nox on a new computer or need a fresh copy.</p>
      <div class="status-list"><div class="status-line"><span>Viewing</span><b>Original website</b></div><div class="status-line"><span>Metadata</span><b>Live page content script</b></div><div class="status-line"><span>Related videos</span><b>Best-effort DOM scan</b></div><div class="status-line"><span>Package</span><b>Companion v1.3.3</b></div></div>
      <div class="form-actions" style="justify-content:flex-start;margin-top:12px"><a class="btn primary" href="./downloads/nox-vault-companion-extension-legacy-v1.3.3.zip" download>⇩ Download Legacy Chrome Companion (.zip)</a><button class="btn ghost" data-extension-settings>Extension Settings</button></div>
      <p class="muted smalltext" style="margin-top:10px">After downloading: extract the ZIP → open <b>chrome://extensions</b> → enable Developer mode → <b>Load unpacked</b> → choose the extracted extension folder.</p>
    </section>
    <section class="card setting-group"><h3>Privacy</h3>
      ...
      <div class="setting-row"><div><strong>NSFW</strong><p>Blur thumbnails and stored images for the current vault only.</p></div><input id="set-vault-nsfw" class="toggle" type="checkbox" ${currentVault()?.nsfw?'checked':''}></div>
    </section>
    ...
    <section class="card setting-group danger-zone"><h3>Vault Security</h3><p class="muted smalltext">A vault password is optional. When present, it is PBKDF2-hashed in your owner-only Firestore data. It is an application lock, not encryption.</p><div class="quick-grid"><button class="quick" data-change-pass><strong>🔑 Password</strong><small>Add, change or remove</small></button><button class="quick" data-lock-now><strong>🔒 Lock Now</strong><small>${vaultHasPassword(currentVault())?'Lock current vault':'No password configured'}</small></button></div></section>
  </div>`;
  ...
  $('#set-vault-nsfw').onchange=async e=>{await backend.updateVault(S.currentVaultId,{nsfw:e.target.checked});const v=byId(S.vaults,S.currentVaultId);if(v)v.nsfw=e.target.checked;applyVaultPrivacy();toast(`NSFW preview blur ${e.target.checked?'enabled':'disabled'} for this vault.`,'success')};
...
function renderPrivacy(){
  ...
  <div class="settings-grid">
    ...
    <section class="card setting-group"><h3>Browser Companion</h3><p class="muted smalltext">Normal viewing uses the Chrome extension in-page sidebar over the real website. This works on older Chrome versions and avoids iframe/CORS restrictions. This avoids iframe/CORS restrictions and lets Nox read metadata from the live page through a content script.</p><button class="btn ghost" onclick="location.hash='#extension'">Companion status</button></section>
  </div>`;

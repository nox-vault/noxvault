// Nox Vault v1.1 playback/session/decoy update 2026-09-15
import {backend} from './backend-v6.js';
import {$,$$,esc,fmtDate,timeAgo,uid,nowIso,normalizeUrl,sourceFromUrl,isDirectVideo,isHls,parseTags,shortcutMatches,prettyShortcut,downloadBlob,pickFile,sortBy,randomSubset} from './utils-v6.js';
...
function normalizeSession(s){
  const tabs=(s.tabs||[]).map(t=>({
    id:t.id||uid(),url:t.url||'',title:t.title||sourceFromUrl(t.url),
    embedUrl:t.embedUrl||'',mediaUrl:t.mediaUrl||'',thumbnail:t.thumbnail||'',
    ...
    pinned:!!t.pinned,lastActiveAt:t.lastActiveAt||0,tags:t.tags||[],categoryIds:t.categoryIds||[]
  }));
...
function playbackChoice(tab){
  ...
  if(mode==='webpage')return {kind:'iframe',url:tab.url||''};
  if(mode==='embed')return {kind:'iframe',url:tab.embedUrl||tab.url||''};
  if(mode==='direct'){
    const u=tab.mediaUrl||tab.url||'';
    return {kind:'video',url:u};
  }
  const direct=tab?.mediaUrl||'';
  if(direct&&(isDirectVideo(direct)||isHls(direct)))return {kind:'video',url:direct};
  if(S.settings?.preferDetectedPlayer!==false&&tab?.embedUrl)return {kind:'iframe',url:tab.embedUrl};
  return {kind:'iframe',url:tab?.url||tab?.embedUrl||''};
}
function renderRelated(related=[]){
  if(!related.length)return '<p class="muted smalltext">No related links could be detected. If the source page is loaded in the main iframe, its own related section remains available there.</p>';
...
function renderSessionView(){
  ...
  <div class="session-layout">
    <section class="card browser-card">
      ...
      <div class="browser-modebar">
        ...
        ${current?.embedUrl?badge('Embed detected','green'):badge('No embed detected','purple')}
        ${current?.mediaUrl?badge('Media detected','green'):''}
        ${choice?.url?`<a class="btn small ghost" href="${esc(current.url)}" target="_blank" rel="noopener noreferrer">Open source ↗</a>`:''}
      </div>
    ...
    <aside class="session-side">
      ...
      <section class="card side-section"><h3>Related from source</h3><div class="related-list">${renderRelated(current?.related||[])}</div></section>
  ...
  $$('[data-related]',page).forEach(b=>b.onclick=async()=>{
    const r=current?.related?.[Number(b.dataset.related)];
    if(!r?.url)return;
    const t={id:uid(),url:r.url,title:r.title||sourceFromUrl(r.url),thumbnail:r.thumbnail||'',embedUrl:'',mediaUrl:'',playbackMode:'auto',related:[],status:'suspended',pinned:false,lastActiveAt:0,tags:[],categoryIds:[]};
...
function addTabModal(){modal(`<h2>Add Tab</h2><p>Enter a normal webpage URL. Nox Vault will also try to detect an official embed/direct media URL automatically.</p><form class="form-grid"><label class="full">URL<input name="url" type="url" required placeholder="https://example.com/video"></label><label>Optional title<input name="title" placeholder="Auto-detect"></label><label>Optional embed URL<input name="embedUrl" type="url" placeholder="https://…/embed/…"></label><div class="form-actions full"><button class="btn primary">Add Tab</button></div></form>`,{onOpen:r=>r.querySelector('form').onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));const t={id:uid(),url:d.url,title:d.title||sourceFromUrl(d.url),embedUrl:d.embedUrl||'',mediaUrl:isDirectVideo(d.url)||isHls(d.url)?d.url:'',playbackMode:'auto',related:[],metadataCheckedAt:null,status:'suspended',pinned:false,lastActiveAt:0,tags:[],categoryIds:[]};S.activeSession.tabs.push(t);r.remove();await activateTab(t.id)}})}
async function navigateCurrent(url){
  const t=byId(S.activeSession?.tabs,S.currentTabId);if(!t||!url)return;
  t.url=url;t.mediaUrl=isDirectVideo(url)||isHls(url)?url:'';t.embedUrl='';t.title=sourceFromUrl(url);
  t.related=[];t.metadataCheckedAt=null;t.playbackMode='auto';t.status='active';t.lastActiveAt=Date.now();
  destroyTabFrame(t.id);await saveActiveSession(false);renderSessionView();queueAutoDetect(t);
}
...
async function metadataIntoTab(tab,announce=true){
  ...
  if(announce)toast('Detecting player, title, tags and related links…');
  try{
    ...
    if(announce)toast(tab.embedUrl||tab.mediaUrl?'Player/metadata detected.':'Metadata checked; using webpage fallback.','success');
  }catch(err){
...
function playerStage(i){
  ...
  return `<iframe id="playlist-frame" src="${esc(choice.url)}" allow="autoplay; fullscreen; picture-in-picture; encrypted-media" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>`;
}
...
function renderSettings(){
  ...
  <div class="settings-grid">
    ...
    <section class="card setting-group"><h3>Playback Detection</h3>
      <div class="setting-row"><div><strong>Auto-detect embedded player</strong><p>When a tab opens, inspect accessible metadata for embed/direct-player URLs.</p></div><input id="set-auto-detect" class="toggle" type="checkbox" ${S.settings.autoDetectPlayback!==false?'checked':''}></div>
...
function renderPrivacy(){
  ...
  <div class="settings-grid">
    ...
    <section class="card setting-group"><h3>Embedding limits</h3><p class="muted smalltext">Nox can load normal pages or detected players in iframes, but websites may block embedding with X-Frame-Options or CSP. The app cannot override those browser-enforced restrictions.</p></section>
  </div>`;

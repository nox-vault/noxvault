...
import {shortcutMatches} from './utils-v8.js';

const $=s=>document.querySelector(s);
const state={settings:null,currentVaultId:localStorage.getItem('nox_current_vault')||'',sections:[],defaultIndex:0,user:null,vault:null};
const vaultHasPassword=v=>Boolean(v&&(v.lockHash||v.hasPassword));
let unlockTimer=null,unlockSeq=0;
...
function returnToNox(){
  if(state.currentVaultId)sessionStorage.setItem('nox_decoy_unlocked_vault',state.currentVaultId);
  const ret=sessionStorage.getItem('nox_return_hash')||'#dashboard';
  location.href=`./index.html${ret.startsWith('#')?ret:'#dashboard'}`;
}
async function attemptUnlock({showWrong=false,button=null}={}){
  const seq=++unlockSeq,msg=$('#decoy-message'),input=$('#decoy-password');
  msg.textContent='';
  if(!state.user){msg.textContent='Your Firebase session is not signed in. Return to the login page.';return false}
  if(!state.currentVaultId){msg.textContent='No current vault is selected.';return false}
  if(state.vault&&!vaultHasPassword(state.vault)){returnToNox();return true}
  const value=input.value;
  if(value.length<2){if(showWrong)msg.textContent='Enter at least 2 characters.';return false}
  if(button){button.disabled=true;button.textContent='Checking…'}
  try{
    const ok=await backend.verifyVault(state.currentVaultId,value);
    if(seq!==unlockSeq)return false;
    if(ok){returnToNox();return true}
    if(showWrong)msg.textContent='Incorrect vault password.';
    return false;
  }catch(err){if(seq===unlockSeq)msg.textContent=err.message||String(err);return false}
  finally{if(button&&seq===unlockSeq){button.disabled=false;button.textContent='Unlock'}}
}
function showUnlock(){
  if(state.vault&&!vaultHasPassword(state.vault)){returnToNox();return}
  $('#decoy-modal').classList.remove('hidden');
  const input=$('#decoy-password');input.value='';
  $('#decoy-message').textContent='';
  requestAnimationFrame(()=>input.focus());
}
function hideUnlock(){clearTimeout(unlockTimer);$('#decoy-modal').classList.add('hidden')}
$('#decoy-unlock').onclick=showUnlock;
$('#decoy-cancel').onclick=hideUnlock;
$('#decoy-password').addEventListener('input',()=>{
  clearTimeout(unlockTimer);
  $('#decoy-message').textContent='';
  const value=$('#decoy-password').value;
  if(value.length<2)return;
  unlockTimer=setTimeout(()=>attemptUnlock({showWrong:false}),320);
});
$('#decoy-unlock-form').onsubmit=async e=>{
  e.preventDefault();
  await attemptUnlock({showWrong:true,button:e.submitter});
};
...
backend.onAuth(async(user,error)=>{
  ...
  if(user){
    try{
      ...
      state.vault=vaults.find(v=>v.id===state.currentVaultId)||null;

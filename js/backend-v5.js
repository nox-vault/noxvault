...
import { OWNER_UID } from './firebase-config.js?v=20260914-1735';
import { DEFAULT_CATEGORIES,uid,nowIso,normalizeUrl,sha256,sourceFromUrl } from './utils-v5.js';
...
function inferMetadata(url){
  ...
  return {title,source,thumbnail:'',embedUrl:'',mediaUrl:'',tags,suggestedCategories};
}
...
function parsePageMetadata(html,url,base){
  ...
  const thumbnail=abs(meta('og:image','twitter:image'));
  const mediaUrl=abs(meta('og:video:secure_url','og:video:url','og:video'));
  const keywords=meta('keywords').split(',').map(x=>x.trim()).filter(Boolean);
  const merged=inferMetadata(`${url} ${title} ${keywords.join(' ')}`);
  return {...base,...merged,title,thumbnail,mediaUrl,tags:[...new Set([...(base.tags||[]),...keywords,...merged.tags])].slice(0,30),suggestedCategories:[...new Set([...(base.suggestedCategories||[]),...merged.suggestedCategories])]};
}

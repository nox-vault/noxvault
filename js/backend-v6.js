...
import { OWNER_UID } from './firebase-config.js?v=20260915-0432';
import { DEFAULT_CATEGORIES,uid,nowIso,normalizeUrl,sha256,sourceFromUrl } from './utils-v6.js';
...
function inferEmbedUrl(url){
  try{
    ...
    if(/\/(embed|player)\//i.test(u.pathname)||/xembed\.php/i.test(u.pathname))return u.href;

    // Pornhub page URLs expose a stable viewkey that can be converted to the site's
    // normal embed route without scraping the page.
    if(host.endsWith('pornhub.com')){
      const key=u.searchParams.get('viewkey');
      if(key)return `https://www.pornhub.com/embed/${encodeURIComponent(key)}`;
    }

    // Some xHamster links expose a numeric video id as a query parameter. If present,
    // use the site's documented-style xembed endpoint. Modern slug-only links normally
    // need page metadata (JSON-LD/iframe markup) to discover the player URL.
    if(host.endsWith('xhamster.com')){
      const id=u.searchParams.get('video')||u.searchParams.get('id');
      if(id&&/^\d+$/.test(id))return `https://xhamster.com/xembed.php?video=${encodeURIComponent(id)}`;
      const modernCode=u.pathname.match(/-(xh[a-z0-9]+)\/?$/i)?.[1];
      if(modernCode)return `https://xhamster.com/embed/${encodeURIComponent(modernCode)}`;
      const numericCode=u.pathname.match(/-(\d+)\/?$/)?.[1];
      if(numericCode)return `https://xhamster.com/embed/${encodeURIComponent(numericCode)}`;
    }
  }catch{}
  return '';
}
...
export const backend={
  ...
  async fetchMetadata(url){
    ...
    // No Cloud Functions: try a direct CORS-safe page fetch. Many sites block this,
    // in which case Nox Vault still provides URL/title/tag/category heuristics locally.
    try{
      const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),4500);
      const r=await fetch(url,{mode:'cors',credentials:'omit',referrerPolicy:'no-referrer',signal:ctl.signal});clearTimeout(timer);
      const type=r.headers.get('content-type')||'';
      if(r.ok&&type.includes('text/html'))return parsePageMetadata(await fetchTextLimited(r),url,base);
    }catch{}
    return base;
  },
